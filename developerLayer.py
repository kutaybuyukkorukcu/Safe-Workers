...
class Checker:
    ...
    def is_in_boundary(self, boxes, coords):
        # box[0] -> y0, box[1] -> x0, box[2] -> y1, box[3] -> x1
        return boxes[0] <= coords[0] and boxes[1] <= coords[1] and boxes[2] >= coords[2] and boxes[3] >= coords[3]

    def is_in_checklist(self, detections, checklist, boxes):
        th = 0.6
        liste = []
        for element in checklist:
            ...
            while i < detections['meta'][element] and flag is False:
                if detections['objects'][element][i][0] >= th and \
                        self.is_in_boundary(boxes, detections['objects'][element][i][1]):
